...
const postToGit =  async(jsxData, htmlData, newFilePath, name, heText, author, description,createFolder, setModalLoading, store) => {
  ...
  try { // post changes to git
    const response = await fetch('https://git-api-push.vercel.app/update', {
      ...
      body: JSON.stringify({ jsxData, newFilePath, name, heText, author, description, createFolder }),
    });
